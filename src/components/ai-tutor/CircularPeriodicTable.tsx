...
const CircularPeriodicTable: React.FC = () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      import('p5').then((p5Module) => {
        ...
        new p5((p) => {
          ...
          let cellSize = 80;
          let electronOrbitals: any[] = [];
          let electronSpeed = 0.02;
          let categoryColors: Record<string, any> = {};
          ...
          p.draw = () => {
            ...
            p.textSize(32);
            p.textStyle(p.BOLD);
            p.text("Interactive Periodic Table with Atom Visualization", p.width/2, 50);
            ...
            p.text("Click on an element to view its atomic structure. Click in empty space to reset.", p.width/2, p.height - 40);
          };
          ...
          const drawDefaultAtom = () => {
            ...
            p.fill(255);
            p.textSize(18);
            p.text("Click an element to view its atomic structure", x, y + 140);
          };
          ...
          const drawCircularElements = () => {
            ...
            const radius = p.min(p.width, p.height) * 0.38; // Circular radius for elements
          ...
          const drawElementDetails = () => {
            ...
            p.text(`Electron Configuration: ${selectedElement.electronConfig}`, leftColX, infoY + infoSpacing * 2);
            p.text(`Melting Point: ${selectedElement.meltingPoint} K`, leftColX, infoY + infoSpacing * 3);
            
            // Right column
            let rightColX = p.width/2 + 50;
            
            p.text(`Boiling Point: ${selectedElement.boilingPoint} K`, rightColX, infoY);
            p.text(`Discovered: ${selectedElement.discoveryYear} (${selectedElement.discoveredBy})`, rightColX, infoY + infoSpacing);
            
            // Common uses (spans both columns)
            p.text(`Uses: ${selectedElement.uses.substring(0, 100)}...`, leftColX, infoY + infoSpacing * 4);
